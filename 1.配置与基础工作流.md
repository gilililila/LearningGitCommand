...
git init
```
运行命令，我们会看到 ![运行git init](LearningGitCommand/images/git_init.png "运行git init后结果")
说明我们成功初始化了一个Git仓库！
...
![](LearningGitCommand/images/create_file.png)
...
git add .
```
此时使用`git status`命令可以查看上传的文件。![](LearningGitCommand/images/fstadd_status.png)

2. **提交到本地仓库**
	同样是在根目录下运行，将提交到暂存区的代码永久保存。`-m`后的描述可以根据情况修改，具体见。
```Bash
...
使用`git log`命令可以查看详细的提交历史，包括提交编号、位置指针、提交作者、提交日期以及提交说明。![](LearningGitCommand/images/fstcmt_log.png)
